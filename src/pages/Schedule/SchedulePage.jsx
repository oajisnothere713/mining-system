...
function shiftToCurrentWeek(dateStr) {
  const diffDays = Math.round((new Date(dateStr).getTime() - new Date("2026-06-01T00:00:00").getTime()) / (1000 * 60 * 60 * 24));
  const today = new Date();
  const dDay = today.getDay();
  const monday = new Date(today);
  monday.setDate(today.getDate() - dDay + (dDay === 0 ? -6 : 1));
...
const B = B_raw.map(b => ({
  ...b,
  date: shiftToCurrentWeek(b.date),
  ...(b.multiDay ? { multiDay: { from: shiftToCurrentWeek(b.multiDay.from), to: shiftToCurrentWeek(b.multiDay.to) } } : {})
}));

function getMonthName(dStr) { return ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][parseInt(dStr.slice(5,7)) - 1]; }
      ...
      >
        ...
        {(b.recurrence || b.multiDay) && (
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 9.5, fontWeight: 600, color: O, background: OS, padding: '2px 6px', borderRadius: 5, marginBottom: 5 }}>
            ...
            {b.multiDay ? `Multi-day · ${b.multiDay.from.slice(8)}–${b.multiDay.to.slice(8)} ${getMonthName(b.multiDay.from)}` : b.recurrence}
          </div>
        )}
        ...
        <div className="panel-content">
          <div style={{ padding: '18px 22px', borderBottom: `1px solid ${LN}`, flexShrink: 0 }}>
            ...
            {(b.recurrence || b.multiDay) && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11.5, fontWeight: 600, color: O, background: OS, padding: '4px 10px', borderRadius: 6, marginTop: 10 }}>
                ...
                {b.multiDay ? `Multi-day booking · ${b.multiDay.from.slice(8)}–${b.multiDay.to.slice(8)} ${getMonthName(b.multiDay.from)} ${b.multiDay.from.slice(0,4)}` : `Recurring · ${b.recurrence}`}
              </div>
            )}
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '20px 22px' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: `1px solid ${BG}`, fontSize: 13 }}>
                <span style={{ color: SL }}>{b.multiDay ? "Dates" : "Date & Time"}</span><span style={{ color: INK, fontWeight: 600, textAlign: 'right' }}>{b.multiDay ? `${b.multiDay.from.slice(8)}–${b.multiDay.to.slice(8)} ${getMonthName(b.multiDay.from)} · ${b.time}` : `${b.date.slice(8)} ${getMonthName(b.date)} ${b.date.slice(0,4)} · ${b.time}`}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: `1px solid ${BG}`, fontSize: 13 }}>
    ...
    <div className="schedule-container">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={() => setBaseDate(() => { const t = new Date(); const d = t.getDay(); const diff = t.getDate() - d + (d === 0 ? -6 : 1); const monday = new Date(t.setDate(diff)); monday.setHours(0,0,0,0); return monday; })} style={{ height: 36, padding: '0 14px', borderRadius: 9, border: `1px solid ${O}`, background: OS, fontWeight: 600, fontSize: 13.5, color: O, cursor: 'pointer' }}>Today</button>
        </div>
      </div>
